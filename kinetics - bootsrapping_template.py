...
wd = "C:\\Users\\uthma\\Dropbox (UFL)\\PhD\\Lab analysis result\\CityBlock15cm.csv"
pd.read_csv(wd)
data = pd.read_csv(wd)
#Conc = [1.4382,3.1203,6.0669,7.7319,9.2007]

#dat = data[(data["C0"])]
t = np.array(data['Time'])
ymeas = np.array(data['Ce_C0'])
...
def model(p):
    ...
    #f = p[2]
    # predicted values
    f=0 #means no instantaneous sorption sites
    m=1
    v=1
    r=1+(m/v)*k
    b=(1+f*(m/v)*k)/r
...
plt.legend(loc='best')
plt.savefig('optimizedalldata.png',dpi=320)
plt.show()
